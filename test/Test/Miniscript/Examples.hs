...
import           Language.Bitcoin.Miniscript (Annotation (..), Miniscript (..),
                                              literal, older, thresh, var, (.:))
...
example7 = Example
    ...
    , script = T .: (keyVar "key_revocation" `OrC` AndV (V .: keyVar "key_remote")
...
example8 = Example
    ...
    , script = AndOr (keyVar "key_remote")
