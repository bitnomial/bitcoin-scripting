...
satisfactionContext preimages sigs = SatisfactionContext
...
satisfy' chainState sc = \case
    Boolean False -> return SatResult { sat  = Left Impossible
                                      , dsat = Right mempty
                                      }

    Boolean True -> return SatResult { sat  = Right mempty
    ...
    Key vk -> withLiteral guardKey satisfyKey vk
        ...
        satisfyKey k
            ...
            | otherwise = return SatResult { sat  = Left $ MissingSignature [k]
    ...
    KeyH vk -> withLiteral guardKey satisfyKeyH vk
        ...
        satisfyKeyH k
            | Just pk <- keyDescPubKey k
            , Just s  <- lookupSignature pk sc
            = satVals (fromScript [pushSig s, pushKey pk])
                      (fromScript [OP_0, pushKey pk])

            | Just pk <- keyDescPubKey k
            = return SatResult { sat  = Left $ MissingSignature [k]
    ...
    OrI x z -> satOrI <$> satisfyInContext x <*> satisfyInContext z
        where
        satOrI sx sz = SatResult
            { sat = let satA = (<> SatScript 1 [OP_1]) <$> sat sx
                        satB = (<> SatScript 1 [OP_0]) <$> sat sz
                    in satA `satOr` satB
            , dsat = let dsatA = (<> SatScript 1 [OP_1]) <$> dsat sx
    ...
    AnnD x -> revise <$> satisfyInContext x
        where
        revise s = s { sat = (<> SatScript 1 [OP_1]) <$> sat s
                     , dsat = return $ SatScript 1 [OP_0]
                     }
